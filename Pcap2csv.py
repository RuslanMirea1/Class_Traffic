...
import pandas as pd
import subprocess
import re  # Добавляем обратно импорт модуля re
...
def ip_from_string(ips):
    return bytes(int(n) for n in ips.split("."))

def parse_flows(pcapfile):
    pipe = subprocess.Popen(["./ndpiReader", "-i", pcapfile, "-v2"], stdout=subprocess.PIPE)
...
def forge_flow_stats(flow, strip=0):
    ...
    if isinstance(seg, dpkt.tcp.TCP):
        ...
        raise ValueError("Unknown transport protocol: `{}`".format(seg.__class__.__name__))
    ...
    stats.update({
        ...
        "client_efficiency": sum(client_bulks) / sum(client_packets),
        ...
        for proto, subproto, flow in parse_flows(pcapfile):
            ...
            if stats:
                ...
                for feature in FEATURES:
                    flows[feature].append(stats.get(feature, None))
